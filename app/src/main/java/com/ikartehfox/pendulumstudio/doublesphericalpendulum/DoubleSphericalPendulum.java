...
public class DoubleSphericalPendulum extends GenericPendulum {
    ...
    void accel2(double[] a, double[] qt, double[] qvt) {
        ...
        bcoefs[0] = (m1 + m2) * l1 * l1 * sinth1 * costh1 * qvt[1] * qvt[1]
                ...
                - cosdphi * costh1 * sinth2 * qvt[3] * qvt[3]);
        if (coordSystem == 0)
            bcoefs[0] += (m1 + m2) * l1 * (gx * cosph1 * costh1 + gy * sinph1 * costh1 - gz * sinth1);
        else if (coordSystem == 1)
            bcoefs[0] += (m1 + m2) * l1 * (gz * cosph1 * costh1 + gx * sinph1 * costh1 - gy * sinth1);
        else
            bcoefs[0] += (m1 + m2) * l1 * (gy * cosph1 * costh1 + gz * sinph1 * costh1 - gx * sinth1);
        bcoefs[0] -= 2 * k * l1 * l1 * qvt[0] + k * l1 * l2 * (sindphi * costh1 * sinth2 * qvt[3]
                + (cosdphi * costh1 * costh2 + sinth1 * sinth2) * qvt[2]);
        bcoefs[1] = -(m1 + m2) * l1 * l1 * 2. * sinth1 * costh1 * qvt[0] * qvt[1]
                - m2 * l1 * l2 * (sindphi * sinth1 * sinth2 * qvt[3] * qvt[3]
                ...
                + sindphi * sinth1 * sinth2 * qvt[2] * qvt[2]);
        if (coordSystem == 0)
            bcoefs[1] += -(m1 + m2) * gx * l1 * sinph1 * sinth1 + (m1 + m2) * gy * l1 * cosph1 * sinth1;
        else if (coordSystem == 1)
            bcoefs[1] += -(m1 + m2) * gz * l1 * sinph1 * sinth1 + (m1 + m2) * gx * l1 * cosph1 * sinth1;
        else
            bcoefs[1] += -(m1 + m2) * gy * l1 * sinph1 * sinth1 + (m1 + m2) * gz * l1 * cosph1 * sinth1;
        bcoefs[1] -= 2 * k * l1 * l1 * sinth1 * sinth1 * qvt[1] + k * l1 * l2 * (cosdphi * sinth1 * sinth2 * qvt[3]
                - sindphi * sinth1 * costh2 * qvt[2]);
        bcoefs[2] = m2 * l2 * l2 * sinth2 * costh2 * qvt[3] * qvt[3]
                - m2 * l1 * l2 * (-2. * sindphi * costh1 * costh2 * qvt[1] * qvt[0]
                ...
                - cosdphi * sinth1 * costh2 * qvt[1] * qvt[1]);
        if (coordSystem == 0)
            bcoefs[2] += m2 * l2 * (gx * cosph2 * costh2 + gy * sinph2 * costh2 - gz * sinth2);
        else if (coordSystem == 1)
            bcoefs[2] += m2 * l2 * (gz * cosph2 * costh2 + gx * sinph2 * costh2 - gy * sinth2);
        ...
        bcoefs[3] = -m2 * l2 * l2 * 2. * sinth2 * costh2 * qvt[2] * qvt[3]
                ...
                - sindphi * sinth1 * sinth2 * qvt[0] * qvt[0]);
        if (coordSystem == 0)
            bcoefs[3] += -m2 * gx * l2 * sinph2 * sinth2 + m2 * gy * l2 * cosph2 * sinth2;
        else if (coordSystem == 1)
            bcoefs[3] += -m2 * gz * l2 * sinph2 * sinth2 + m2 * gx * l2 * cosph2 * sinth2;
    ...
    double poen() {
        if (coordSystem == 0)
            return (-m1 * g * l1 * Math.cos(q[0]) - m2 * g * (l1 * Math.cos(q[0]) + l2 * Math.cos(q[2]))) / 10000.0;
        else if (coordSystem == 1)
            return (-m1 * g * l1 * Math.cos(q[1]) * Math.sin(q[0]) - m2 * g * (l1 * Math.cos(q[1]) * Math.sin(q[0]) + l2 * Math.cos(q[3]) * Math.sin(q[2]))) / 10000.0;
        else
            return (-m1 * g * l1 * Math.sin(q[1]) * Math.sin(q[0]) - m2 * g * (l1 * Math.sin(q[1]) * Math.sin(q[0]) + l2 * Math.sin(q[3]) * Math.sin(q[2]))) / 10000.0;
    }
    ...
    double getxv1() {
        if (coordSystem == 0)
            return -l1 * Math.sin(q[1]) * Math.sin(q[0]) * qv[1] + l1 * Math.cos(q[1]) * Math.cos(q[0]) * qv[0];
        else if (coordSystem == 1)
            return l1 * Math.cos(q[1]) * Math.sin(q[0]) * qv[1] + l1 * Math.sin(q[1]) * Math.cos(q[0]) * qv[0];
    ...
    double getx2() {
        if (coordSystem == 0)
            return l1 * Math.cos(q[1]) * Math.sin(q[0]) + l2 * Math.cos(q[3]) * Math.sin(q[2]);
        else if (coordSystem == 1)
            return l1 * Math.sin(q[1]) * Math.sin(q[0]) + l2 * Math.sin(q[3]) * Math.sin(q[2]);
    ...
    double getxo2() {
        if (coordSystem == 0)
            return l1 * Math.cos(qo[1]) * Math.sin(qo[0]) + l2 * Math.cos(qo[3]) * Math.sin(qo[2]);
        else if (coordSystem == 1)
            return l1 * Math.sin(qo[1]) * Math.sin(qo[0]) + l2 * Math.sin(qo[3]) * Math.sin(qo[2]);
    ...
    double getyv2() {
        if (coordSystem == 0)
            ...
            return -l1 * Math.sin(q[0]) * qv[0] - l2 * Math.sin(q[2]) * qv[2];
        else
            return -l1 * Math.sin(q[1]) * Math.sin(q[0]) * qv[1] + l1 * Math.cos(q[1]) * Math.cos(q[0]) * qv[0]
                    - l2 * Math.sin(q[3]) * Math.sin(q[2]) * qv[3] + l2 * Math.cos(q[3]) * Math.cos(q[2]) * qv[2];
    }
