...
public class SP2DGLActivity extends Activity implements SensorEventListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        useDamping = Math.abs(SP2DGLRenderer.mPendulum.gam) > 1.e-7;
...
// thread:
            mGLView.queueEvent(() -> SP2DGLRenderer.mPendulum.paused = !isRunning);
            if (isRunning) {
                SP2DGLRenderer.mPendulum.frames = 0;
                deltaT = System.currentTimeMillis();
...
// thread:
        findViewById(R.id.button_restart).setOnClickListener(v -> mGLView.queueEvent(() -> {
            ...
            if (useDamping)
                SP2DGLRenderer.mPendulum.gam = SP2DSimulationParameters.simParams.gam;
...
// thread:
        ...
        findViewById(R.id.togglebutton_sensor_gravity).setOnClickListener(v -> {
            ...
            if (useDynGravity)
                mSensorManager.registerListener(SP2DGLActivity.this, mGravity, SensorManager.SENSOR_DELAY_GAME);
...
// thread:
        ...
        ((ToggleButton) findViewById(R.id.togglebutton_trace)).setChecked(SP2DSimulationParameters.simParams.showTrajectory);
    ...
    public final void onSensorChanged(SensorEvent event) {
        ...
        if (rotmode == Surface.ROTATION_0)
            SP2DGLRenderer.mPendulum.setGravity(event.values[0], event.values[1], event.values[2]);
        else if (rotmode == Surface.ROTATION_90)
            SP2DGLRenderer.mPendulum.setGravity(-event.values[1], event.values[0], event.values[2]);
    ...
    protected void onResume() {
        ...
        if (SP2DSimulationParameters.simParams.showTrajectory && !((ToggleButton) findViewById(R.id.togglebutton_trace)).isChecked())
            // This method will be called on the rendering
// thread:
            mGLView.queueEvent(() -> SP2DGLRenderer.mPendulum.clearTrajectory());
        ((ToggleButton) findViewById(R.id.togglebutton_trace)).setChecked(SP2DSimulationParameters.simParams.showTrajectory);

        // This method will be called on the rendering
// thread:
        mGLView.queueEvent(() -> SP2DGLRenderer.mPendulum.setColorPendulum1(SP2DSimulationParameters.simParams.pendulumColor));
    ...
    protected void setFullScreenMode() {
        ...
        if (full_screen) {
            ...
            if (Build.VERSION.SDK_INT == 16) { // API 16 (Jellybean)
                ...
                SP2DGLSurfaceView glSurfaceView = findViewById(R.id.gl_surface_view);
